...
| `NEXUSAUTH_WORKBENCH_AUTH_CLIENT_SECRET` | 每次启动必填，无默认 | 默认应用共享密钥，由 API 同步到数据库；不进入前端。 |
| `NEXUSAUTH_WORKBENCH_AUTH_REDIRECT_URI` | 本部署覆盖为 http://localhost:5560/signin-oidc；代码默认 http://localhost:5051/signin-oidc | 精确的授权回调地址。 |
| `NEXUSAUTH_WORKBENCH_AUTH_POST_LOGOUT_REDIRECT_URI` | 本部署覆盖为 http://localhost:5560/；代码默认 http://localhost:5273/ | 登出后 Dashboard 地址。 |
| `NEXUSAUTH_WORKBENCH_AUTH_SCOPE` | 可选，openid profile offline_access nexusauth.workbench.api | OIDC 请求 Scope；改变 Audience 时同步修改。 |
...
| `NEXUSAUTH_WORKBENCH_AUTH_SIGN_OUT_PROVIDER` | 可选，true | Dashboard 登出是否同时结束 Provider 会话。 |
